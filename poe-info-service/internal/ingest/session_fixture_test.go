...
func TestFullSessionFixture_IngestsWithoutError(t *testing.T) {
	...
	checks := []struct {
		...
		{"session_afk row (afk)", `SELECT COUNT(*) FROM session_afk WHERE kind='afk'`},
		{"session_afk row (alt-tab)", `SELECT COUNT(*) FROM session_afk WHERE kind='alt_tab' AND afk_off_at IS NOT NULL`},
