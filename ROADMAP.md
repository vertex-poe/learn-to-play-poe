...
- [ ] Decouple poe-info-service from reading l2p-poe.toml directly: `config.LoadChannelNames` (`poe-info-service/config/config.go`) is a hand-rolled partial TOML parser that reaches into l2p-poe's own config file purely to bake `[chat_channel_names]` labels into ingested chat rows — a dependency in the wrong direction, since poe-info-service shouldn't need to understand l2p-poe's file format. Two call sites now take a `--config-path` flag and call it: `ServiceManager::start()` (`src/services/ServiceManager.cpp`) passes it to the persistent service at boot (`main.go`), and the headless dev CLI (`poe-info-service log ingest`, `poe-info-service/cli.go`, added for `poe-info-service/dev/area_seeds/ingest_client_log.py`) takes it as a one-shot flag. For the persistent service, replace with l2p-poe handing channel names over as data (a WS request/method) instead of a path to go parse — boot-time-only today, so labels can't change without restarting the service either. The dev CLI has no live app/WS connection to ask instead, so it likely keeps direct parsing as a deliberate, narrower exception — worth confirming that's still the right call once the WS-based path above exists, rather than leaving `LoadChannelNames` around by accident
