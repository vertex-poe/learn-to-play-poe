...
- [ ] Steam presence: l2p-poe WebView capture of the Steam Web API key — navigate to `steamcommunity.com/dev/apikey` (Steam's own page handles login/2FA in-browser, same as the existing `PoeLoginWindow`/`PoeAccountStore` POESESSID flow), scrape the key from the page once displayed (regex on page HTML, the same trick PlayniteExtensions' SteamStoreService uses to scrape its `webapi_token`), then hand it to poe-info-service via the existing `credentials.store` call (`key: "steamApiKey"`) — no poe-info-service changes needed, this is purely an l2p-poe capture improvement. Unlike Playnite's short-lived `webapi_token`, a Steam Web API key doesn't expire, so this is a one-time capture, not a background-refreshed one. First-time registration (no key yet on the account) still shows Steam's own domain-name/ToS form, which can't be auto-submitted on the user's behalf — capture only fully automates the case where a key already exists or once the user manually completes that one-time form. Currently there is no Accounts-page UI for a Steam key at all, manual-paste or otherwise.
