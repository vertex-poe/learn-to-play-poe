...
- [ ] Account selector (name or uuid) on account-attributable WebSocket requests: not every known account will have a `poe_uuid` — an account seen only via `Client.txt` chat/whisper/guild events (e.g. a friend's account, never OAuth-authenticated on this machine) has no `sub` claim to key on, only a `name`. So requests to any account-attributable read method (chat/whisper/session queries today; stash/characters once the OAuth data endpoints exist) should accept an optional selector that's either a `poe_uuid` or a plain `name`, resolving to whichever `accounts` row matches; omitting the selector defaults to the currently OAuth-active account, keeping the common single-account call path unchanged
